...
lw $t0, 8($gp)  # load 8($gp) 
lw $t1, 4($gp)  # load 4($gp) 
sub $t2, $t1, $t0 # 4($gp)-8($gp)
sw $t2, 0($gp) # store 4($gp)-8($gp) to 0($gp)
add $t2, $t0, $t0 # 8($gp)+8($gp)
sw $t2, 12($gp)# store 8($gp)+8($gp) to 12($gp)
